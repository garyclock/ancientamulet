local function OnTick(inst, target)
    if target.components.health ~= nil and target.components.hunger ~= nil and
      not target.components.health:IsDead() and not target:HasTag("playerghost") then
	target.components.hunger:DoDelta(TUNING.JELLYBEAN_TICK_VALUE)
    else
...
return Prefab("gary_amulet_buff_hungerregen", fn)